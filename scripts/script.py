...
def on_ui_tabs():
        ...
        with gr.Row():
                ...
                with gr.Blocks():
                    ...
                    with gr.Row():
                        worker_update = gr.Button(
                            ...
                            elem_id=f"{tab_prefix}worker-update",
                        )
                    output_textboxes = {}
                    with gr.Column():
                    ...
                    def update_worker_info():
                        new_worker_info = horde_worker.get_worker_info(
                            session, config.apikey, worker
                        )
                        updated_info = {
                            key: new_worker_info.get(key, "")
                            for key in output_textboxes
                        }
                        return updated_info
            ...
            with gr.Tab("News"):
                with gr.Blocks() as news_ui:
                    ...
                    with gr.Row():
                        news_update = gr.Button(
                            "Update News", elem_id=f"{tab_prefix}news-update"
                        )
                        ...
                        with gr.Column():
                            for news_item in news_info[:3]:
                                if (
                                    "title"
                                    and "newspiece"
                                    and "date_published" in news_item
                                ):
                                    gr.Textbox(
                                        news_item["newspiece"],
    ...
    return ((ui, "Stable Horde Worker", "stable-horde"),)
